...
Module FLLMetaTheory (DT : Eqset_dec_pol).
  ...
  Proof.
    ...
    induction w as [| w' IH] using strongind ; intros M B L' L H Hw  ;  destruct L as [|l] ; solveF.
    ...
    + caseLexp l ;inversion Hw as [Hw'];subst;auto;simpl;simpl in H;inversion Hw'; inversion H;subst;solveF;
      ...
      ++ (* par *)
        inversionF H .
        apply IH  with (L:= F :: G :: L) (m:= (Exp_weight  F) + (Exp_weight G) + L_weight L) in H6 ; solveF.  autounfold. lia.
      ++(* with *)
        inversion H ; solveF.
        apply IH with (L:= F :: L) (m:= (Exp_weight  F) + L_weight  L) in H7;autounfold;try(lia) ; solveF.
        apply IH with (L:= G :: L) (m:= (Exp_weight  G) + L_weight  L) in H8;autounfold;try(lia) ; solveF.
      ++ (* forall *)
        inversion H ; solveF.
        simpl; eapply tri_fx;auto; intro x. eapply IH with (L:=Subst FX x  :: L) ; solveF.
  ...
  Proof.
    ...
    generalize dependent M .
    generalize dependent w .
    induction w as [| w' IH] using strongind  ; intros  M B L' L H H' Hw  ;  destruct L as [|l] ; solveF.
    + simpl in Hw. (* Heqw is inconsisten *)
      apply exp_weight0LF in Hw. contradiction.
    + caseLexp l ;inversion Hw as [Hw'];subst;auto;simpl;simpl in H;inversion Hw' ;inversionF H;solveF;subst;
      ...
      ++ (* par *)
        ...
        eapply IH with (L:= F0 :: G0 :: L) (m:= Exp_weight(F0) + Exp_weight(G0) +  L_weight L) ; solveF.
        lia.
      ++ (* with *)
        inversion H ; solveF.
        inversion H' ; solveF.
        apply tri_with .
        eapply IH with (L:= F0 :: L) (m:= Exp_weight(F0) + L_weight L) ; solveF.
        autounfold ; lia.
        eapply IH with (L:= G0 :: L) (m:= Exp_weight(G0) + L_weight L) ; solveF.
        autounfold ; lia.
      ++  (* quest *)
        inversion H ; solveF.
        inversion H';subst ; solveF.
        apply tri_quest.
        eapply IH with (m:= L_weight L) ; solveF.
        lia.
      ++ (* forall  *)
        inversion H ; solveF.
        inversion H' ; solveF.
        ...
        eapply IH with (m:=  Exp_weight  (Subst FX x) + L_weight  L ) ; solveF.
        rewrite <- subs_weight_weak with (x:=x).  lia.
        apply tri_fx. solveF.
  Qed.
  ...
  Proof.
    ...
    induction w as [| w' IH] using strongind; intros   M   B  L'  L  H1 Hw; destruct L as [|l] ; solveF.
  ...
  Proof.
    ...
    induction w as [| w' IH] using strongind;  intros M B L' L Hw  ;  destruct L as [|l] ; solveF.
    + simpl in Hw. (* Heqw is inconsisten *)
      apply exp_weight0LF in Hw. contradiction.
  ...
  Proof.
    ...
    + caseLexp l ; inversion Hw as [Hw'];solveF; 
      ...
      ++ (* bot *) 
        ...
        intro; generalize (H x); intro Hp'; inversion Hp'; subst ; solveF.
        apply IH  with (m:= L_weight  L) in Hp; try(apply tri_bot);try(lia);auto.
      ++ (* par *)
        assert(Hp: forall x,  |-F- B; M; UP ( F :: G :: L ++ [Subst FX x] ++ L')).
        intro; generalize (H x); intro Hp'; inversion Hp'; subst ; solveF. auto.
        apply IH  with (L:= F :: G :: L) (m:= Exp_weight  F + Exp_weight G + L_weight  L) in Hp ; solveF .
        subst. simpl in Hw';inversion Hw';auto.
        lia.
      ++ (* With *)
        assert(Hp1: forall x,  |-F- B; M; UP ( F:: L ++ [Subst FX x] ++ L')).
        intro; generalize (H x); intro Hp1'; inversion Hp1'; subst ; solveF. auto.
        assert(Hp2: forall x,  |-F- B; M; UP ( G:: L ++ [Subst FX x] ++ L')).
        intro; generalize (H x); intro Hp2'; inversion Hp2'; subst ; solveF. auto.
        inversion Hw'. subst.
      ...
      ++  (* quest *)
        ...
        intro; generalize (H x); intro Hp'; inversion Hp'; subst;LexpContr ; solveF. auto.
        apply IH  with (m:= L_weight  L) in Hp;auto.
      ...
      ++ (* forall *)
        ...
        intro; generalize (H x); intro Hp'; inversion Hp'; subst ; solveF . auto.
        apply tri_fx;intro.
        apply IH with (L:= Subst FX0 x :: L) (m:= Exp_weight  (Subst FX0  x) + L_weight  L) ; solveF.
      ...
      ++ (* case 2 *)
        ...
        +++ (* with *)
          eapply IH with (m:= L_weight(F :: L1 ++ a :: L2))
                         (L:=F :: L1 ++ a :: L2)
                         (L' := [a] ++ L1' ++ [F ] ++ L2') in H6 ; solveF.
          eapply IH with (m:= L_weight(G :: L1 ++ a :: L2))
                         (L:=G :: L1 ++ a :: L2)
                         (L' := [a] ++ L1' ++ [G ] ++ L2') in H7 ; solveF.
        ...
        +++ (* copy *)
          eapply IH with (m:= L_weight(L1 ++ a :: L2))(L:=L1 ++ a :: L2) (L' := [a] ++ L1' ++ L2') in H7 ; solveF.
        ...
        +++ (* forall *)
          ...
          intro x.
          eapply IH with (m:= L_weight(Subst FX x :: L1 ++ a :: L2)) (L:=Subst FX x :: L1 ++ a :: L2) ; solveF.
          inversion Heqw. simpl.
          rewrite <- subs_weight_weak with (x:=x) ; solveF.
          eapply EquivAuxForAll with (L:= a::L1'). solveF.
  Qed.
  ...
  Proof.
    ...
    + (* Base *)
      inversionF H.
      apply trih_init2 with (B' := B' ++ [F]); solveF. eauto.
    + (* Inductive *)
      inversion H0;subst ; solveF.
      ++ eapply H in H3;auto.
         eapply H in H4;auto. 
         eapply trih_tensor;eauto.
      ++  eapply H with (B := B ++ [F0]) (F:=F) in H2 ; solveF.
          eapply trih_quest.
          MReplace ( (B ++ [F]) ++ [F0]) ( (B ++ [F0]) ++ [F]) .
          auto.
      ++ eapply H in H4 ; solveF.
  ...
  Proof.
    ...
    + intros. 
      ...
      ++ (* top *)
        exists 0%nat;split ; solveF.
      ++ (* bot *)
        apply IH with (m:= L_weight  L) in H5;auto.
        destruct H5 as [n'  [IHn IHd]].
        exists (S n');split;auto ; solveF. lia.
      ++  (* PAR *)
        apply IH with (m:= Exp_weight  F0 + Exp_weight  G + L_weight  L) in H5 ; solveF.
        destruct H5 as [n' [IHn IHd]].
        exists (S n');split;auto. lia. simpl. lia.
      ++ (* with *)
        apply IH with (m:= Exp_weight  F0 + L_weight  L) in H6 ; solveF.
        apply IH with (m:= Exp_weight  G + L_weight L) in H7 ; solveF.
        destruct H6 as [n' [IHn IHd]].
        destruct H7 as [m' [IHn' IHd']].
        simpl.
        exists (S (Init.Nat.max n' m'));split ; solveF.
        lia.
        autounfold. lia.
        autounfold. lia.
      ++  (* quest *)
      ...
      ++  (* FORALL *)
        ...
        eapply trih_fx.
        intro x.
        generalize (H1' x); intro Hx. solveF.
  Qed.
End FLLMetaTheory.
